...
public class ServiceBan {
    ...
    public void insertBan(ModelDatBan mdb) throws SQLException {
        String sql_ND = "INSERT INTO dsban (idban, tang, tenkh, sdt, slnguoi, ngaydat, giodat, idkh, trangthai) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Da Dat')";
        ...
        pInsert.execute();
        pInsert.close();
        mdb.setTrangThai("Da Dat");
    }

    //sua
    public void UpdateBan(ModelDatBan mdb) throws SQLException {
        String sql = "UPDATE datban SET tenkh=?, SDT=?,slnguoi=?,ngaydat= ?,giodat=?, trangthai='Da Dat', IDKH = ? WHERE idban=? and tang=?";
        ...
        pInsert.setString(7, mdb.getIDBan());
        pInsert.setString(8, mdb.getTang());
    ...
    """;
        ...
        try (Connection conn = cn.getConnection()) {
            ...
            try (PreparedStatement stmtUpdateDsBan = conn.prepareStatement(sqlUpdateDsBan)) {
                stmtUpdateDsBan.setString(1, mdb.getIDBan());
                stmtUpdateDsBan.setString(2, mdb.getSDT());
                stmtUpdateDsBan.setString(3, mdb.getDate());
                stmtUpdateDsBan.executeUpdate();
            }
    ...
    //ktra ngay dat 
    public boolean checkNgayDat(String email, String tang, String sdt) throws SQLException {
        boolean duplicate = false;
        String sql = "SELECT TOP 1 *\n"
                + "FROM datban\n"
                + "WHERE idban = ? \n"
