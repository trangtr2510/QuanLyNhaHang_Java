...
import com.raven.service.PhieuXuatDAO;
...
public class formXuatNL extends javax.swing.JPanel {
    ...
    public formXuatNL(ModelUser user) {
    ...
    public int idPN() {
        ...
        try (Connection conn = DatabaseConnection.getInstance().getConnection()) {
            ...
            String idNL = service.getLatestPhieuXuat();
            ...
            if (idNL != null && !idNL.isEmpty()) {
                try {
                    ...
                    Logger.getLogger(formXuatNL.class.getName()).log(Level.SEVERE, "Lỗi chuyển đổi ID", e);
                }
            }
        } catch (SQLException ex) {
            // Nếu có lỗi khi kết nối CSDL, log lỗi và giữ giá trị mặc định cho ID
            Logger.getLogger(formXuatNL.class.getName()).log(Level.SEVERE, "Lỗi kết nối cơ sở dữ liệu", ex);
        }
    ...
    private void initComponents() {
        ...
        jPanel2.add(btnXuatHang, new org.netbeans.lib.awtextra.AbsoluteConstraints(350, 600, 123, 37));
    ...
    private void addProductActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addProductActionPerformed
        ...
        int maNLInt = Integer.parseInt(maNL); // Chuyển mã nguyên liệu thành số nguyên
        float tongSoLuongTonKho = service.getSLbyID(maNLInt); // Gọi hàm để lấy tổng số lượng tồn kho

        if (soLuong > tongSoLuongTonKho) {
            JOptionPane.showMessageDialog(this, "Số lượng vượt quá tồn kho! Số lượng còn lại: " + tongSoLuongTonKho, "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private void btnXuatHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXuatHangActionPerformed
        ...
        if (tblNhapHang.getRowCount() <= 0) {
            JOptionPane.showMessageDialog(this, "Vui lòng thêm nguyên liệu muốn xuất hàng");
            return;
        }

        try (Connection conn = DatabaseConnection.getInstance().getConnection()) {
            // Lấy ID phiếu xuất vừa tạo
            String idNL = service.getLatestPhieuXuat();
            int id = (idNL == null || idNL.isEmpty()) ? 1 : Integer.parseInt(idNL) + 1;
            ...
            // Xử lý tổng tiền
            tongTienStr = tongTienStr.replace(" VND", "").trim().replace(",", "");
            float tongTien;
            try {
            ...
            // Tạo đối tượng ModelPhieuNhap
            ModelPhieuNhap phieuNhap = new ModelPhieuNhap(id, dateNgayNhap, idND, tongTien);

            // Kiểm tra số lượng tồn kho trước khi xuất hàng
            String sqlCheckStock = "SELECT SoLuong FROM Kho WHERE MaNL = ?";
            try (PreparedStatement psCheckStock = conn.prepareStatement(sqlCheckStock)) {
                for (int i = 0; i < tblNhapHang.getRowCount(); i++) {
                    int maNL = Integer.parseInt(tblNhapHang.getValueAt(i, 1).toString());
                    float soLuong = Float.parseFloat(tblNhapHang.getValueAt(i, 3).toString());

                    // Kiểm tra số lượng tồn kho của nguyên liệu
                    psCheckStock.setInt(1, maNL);
                    ResultSet rs = psCheckStock.executeQuery();

                    if (rs.next()) {
                        float stockQuantity = rs.getFloat("SoLuong");

                        // Kiểm tra nếu số lượng xuất vượt quá tồn kho
                        if (soLuong > stockQuantity) {
                            JOptionPane.showMessageDialog(this, "Số lượng xuất vượt quá số lượng tồn kho của nguyên liệu có mã: " + maNL + " : " + (soLuong - stockQuantity), "Lỗi", JOptionPane.ERROR_MESSAGE);
                            return; // Dừng lại nếu có lỗi
                        }
                    } else {
                        JOptionPane.showMessageDialog(this, "Không tìm thấy nguyên liệu với mã: " + maNL, "Lỗi", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }

                // Thêm phiếu nhập vào cơ sở dữ liệu
                PhieuXuatDAO phieuXuatDAO = PhieuXuatDAO.getInstance();
                int result = phieuXuatDAO.insert(phieuNhap);
                ...
                if (result > 0) {
                    ...
                    String sqlInsert = "INSERT INTO ChiTietPhieuXuat (MaPX, MaNL, SoLuong, DonGia) VALUES (?, ?, ?, ?)";
                    String sqlUpdateKho = "UPDATE Kho SET SoLuong = SoLuong - ? WHERE MaNL = ?";

                    try (PreparedStatement psInsert = conn.prepareStatement(sqlInsert); PreparedStatement psUpdate = conn.prepareStatement(sqlUpdateKho)) {

                        for (int i = 0; i < modelNhapHang.getRowCount(); i++) {
                            int maNL = Integer.parseInt(modelNhapHang.getValueAt(i, 1).toString());
                            float soLuong = Float.parseFloat(modelNhapHang.getValueAt(i, 3).toString());
                            float donGia = Float.parseFloat(modelNhapHang.getValueAt(i, 6).toString().replace(",", "").trim());

                            // Thêm chi tiết phiếu xuất
                            psInsert.setInt(1, maPN);
                            psInsert.setInt(2, maNL);
                            psInsert.setFloat(3, soLuong);
                            psInsert.setFloat(4, donGia);
                            psInsert.addBatch();

                            // Cập nhật tồn kho
                            psUpdate.setFloat(1, soLuong);
                            psUpdate.setInt(2, maNL);
                            psUpdate.addBatch();
                        }
                        ...
                        e.printStackTrace();
                        JOptionPane.showMessageDialog(this, "Lỗi khi lưu chi tiết phiếu xuất hoặc cập nhật kho. Vui lòng thử lại!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                    }

                } else {
                    JOptionPane.showMessageDialog(this, "Lỗi khi xuất hàng. Vui lòng thử lại!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            }

        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Đã xảy ra lỗi. Vui lòng kiểm tra lại thông tin xuất.", "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        // Lấy mã nguyên liệu từ bảng
        String maNL = tblNhapHang.getValueAt(selectedRow, 1).toString(); // Giả sử mã nguyên liệu ở cột 0
        String currentQuantityStr = tblNhapHang.getValueAt(selectedRow, 3).toString(); // Cột 3 là số lượng
        ...
        String newQuantityStr = JOptionPane.showInputDialog(this, "Nhập số lượng cần thay đổi (hiện tại: " + currentQuantity + ")",
        ...
        if (newQuantityStr != null && !newQuantityStr.trim().isEmpty()) {
            try {
                ...
                if (newQuantity > 0) {
                    if (newQuantity > tongSoLuongTonKho) {
                        JOptionPane.showMessageDialog(this, "Số lượng vượt quá tồn kho! Số lượng còn lại: " + tongSoLuongTonKho,
